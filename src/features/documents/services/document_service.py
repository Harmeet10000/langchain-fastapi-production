...
from features.documents.repositories.document_repository import (
    DocumentRepository,
...
class DocumentService:
    ...
    def __init__(
        self, document_repo: DocumentRepository, vector_repo: VectorRepository
    ):
    ...
    async def upload_document(
        ...
        try:
            ...
            await redis_cache.set(
                f"document:{document_id}", doc_info, ttl=86400
            )  # 24 hours
    ...
    async def process_document(
        self, process_request: DocumentProcessRequest, background_tasks: BackgroundTasks
    ) -> DocumentInfo:
        ...
        try:
            ...
            await redis_cache.set(
                f"document:{process_request.document_id}", doc_info, ttl=86400
            )
        ...
        try:
            ...
            if "vector_ids" in doc_info:
                ...
                await self.vector_repo.delete_by_ids(
                    doc_info["vector_ids"], namespace=namespace
                )
    ...
    async def extract_entities(self, document_id: str) -> EntityExtractionResponse:
        ...
        try:
            ...
            entities = await advanced_document_processor.extract_entities(
                doc_info["file_path"]
            )

            return EntityExtractionResponse(document_id=document_id, entities=entities)
