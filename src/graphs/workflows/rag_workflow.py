...
class RAGWorkflow:
    ...
    def _build_workflow(self) -> Graph:
        ...
        # Add nodes
        workflow.add_node(RAGWorkflowSteps.RETRIEVE, self._retrieve_context)
        workflow.add_node(RAGWorkflowSteps.RERANK, self._rerank_results)
        workflow.add_node(RAGWorkflowSteps.GENERATE, self._generate_answer)
        workflow.add_node(RAGWorkflowSteps.VALIDATE, self._validate_answer)
        
        # Define edges
        workflow.set_entry_point(RAGWorkflowSteps.RETRIEVE)
        
        # Add conditional edge for retrieve
        workflow.add_conditional_edges(
            RAGWorkflowSteps.RETRIEVE,
            self._should_rerank,
            {
                "rerank": RAGWorkflowSteps.RERANK,
                "generate": RAGWorkflowSteps.GENERATE,
                "end": END
            }
        )
        
        workflow.add_edge(RAGWorkflowSteps.RERANK, RAGWorkflowSteps.GENERATE)
        workflow.add_edge(RAGWorkflowSteps.GENERATE, RAGWorkflowSteps.VALIDATE)
        
        # Add conditional edge for validation
        workflow.add_conditional_edges(
            RAGWorkflowSteps.VALIDATE,
            self._should_retry,
            {
                "retry": RAGWorkflowSteps.GENERATE,
                "end": END
            }
        )
    ...
    async def _rerank_results(self, state: RAGState) -> RAGState:
        ...
        try:
            ...
            ranked_sources.sort(
                key=lambda x: (x["rerank_score"], x["score"]),
                reverse=True
            )
    ...
    def _should_retry(self, state: RAGState) -> str:
        ...
        if state.get("metadata", {}).get("needs_improvement") and \
           state.get("metadata", {}).get("retry_count", 0) < 2:
